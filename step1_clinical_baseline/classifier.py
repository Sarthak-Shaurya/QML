...
# --- 1. SETUP FAST GPU STATE-VECTOR SIMULATOR ---
def create_quantum_environments(n_features):
    n_qubits = int(np.log2(n_features))
    
    # Change "default.qubit" to "lightning.gpu"
    dev_kernel = qml.device(
        "lightning.gpu", 
        wires=n_qubits
    )
    
    # Device for qDC Hadamard Test
...
print(f"\nFinal 10-Fold Results for 8 Features:")
print(f"qDC AUC:    {qdc8:.4f}")
print(f"sqKSVM AUC: {sq8:.4f}")
print(f"qKSVM AUC:  {qksvm8:.4f}")

print("\n--- RUNNING 16 FEATURES (4 QUBITS) ---")
qdc16, sq16, qksvm16 = run_experiment(16)

print(f"\nFinal 10-Fold Results for 16 Features:")
print(f"qDC AUC:    {qdc16:.4f}")
print(f"sqKSVM AUC: {sq16:.4f}")
print(f"qKSVM AUC:  {qksvm16:.4f}")
